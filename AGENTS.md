# AGENTS.md — kist-action-jest

`@getkist/action-jest` is a **kist action plugin**: an npm package whose
default export is an `ActionPlugin` descriptor that kist core loads to register
actions usable from a `kist.yml` pipeline. Jest test runner action for kist.
...
| --- | --- |
| `JestAction` | Runs a Jest test suite as a pipeline step. |

Runtime dependencies: `jest`.
...
- This plugin runs Jest *and* is tested with Jest. Keep the spawned run
  isolated (own config/cwd) so a nested run cannot pick up this repo's own
  `jest.config.js`.
...
```bash
npm run build
# then, from a project with a kist.yml that uses JestAction:
