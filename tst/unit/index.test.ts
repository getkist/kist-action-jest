...
import plugin, { Action, ActionPlugin, JestAction } from "../../src/index.js";
...
describe("package entry point", () => {
    it("re-exports every action it provides", () => {
        expect(JestAction).toBeDefined();
        expect(new JestAction()).toBeInstanceOf(Action);
    });
    ...
    it("registers its actions by the name pipeline steps use", () => {
        ...
        expect(Object.keys(actions).sort()).toEqual(["JestAction"]);
        expect(new actions.JestAction()).toBeInstanceOf(JestAction);
    });
});
